...
{
  protected agentKey: string = null;
   ...
   * @returns
   */
  async createConfigSignature(url: string, nonce: string = null, timestamp: number = null): Promise<Record<string, any>>
  {
   ...
   * @returns
   */
  async createAgentConfigSignature(agentId: number, url: string, nonce: string = null, timestamp: number = null): Promise<Record<string, any>>
  {
    nonce = nonce || randomString(10);
    timestamp = timestamp || getTimestamp();
    let ticket = await this.getTicket();

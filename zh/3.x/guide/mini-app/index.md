...
🚨 使用本SDK工具包前，请务必熟读 [微信官方文档：小程序](https://developers.weixin.qq.com/miniprogram/dev/framework/)。
...
```js
{
  ...
  // 是否使用 Stable Access Token， 默认为 false
...
有的时候，你可能需要将配置单独写到一个文件中，可以使用 `defineMiniAppConfig` 函数，这样也可以获得编辑器的提示功能。

```js
const { defineMiniAppConfig } = require('node-easywechat');

const config = defineMiniAppConfig({
  // ...
});
```
